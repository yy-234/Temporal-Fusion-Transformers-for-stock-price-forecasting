...
class Embeddings(nn.Module):
    def __init__(self, d_model, vocab=5000):
...
class TransAm(nn.Module):
    def __init__(self, num_layers=1, feature_size=1, dropout=0.1):
        super(TransAm, self).__init__()
        self.model_type = 'Transformer'
        self.src_mask = None
        #self.embedd=Embeddings(feature_size)
        self.pos_encoder = PositionalEncoding(feature_size)#捕捉相对位置关系
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=feature_size, nhead=1, dim_feedforward=49, dropout=dropout)
...
def get_data():
    series = pd.read_csv('Pfizer.csv', usecols=['Close'])
...
#for epoch in range(1, epochs + 1):
    ...
    #val_loss = mseloss(loss, train_data)
    #print('-' * 89)
    #print('loss  {:5.5f}'.format(val_loss))
